...
**Deliverable:** one new prompt file, one new/extended query_mode branch in `backend/app/api/v1/chat.py`, and a short eval set (10–15 hand-picked algorithms-slide questions with known-correct answers) to catch regressions before shipping. **The eval set was actually built on 2026-09-15, five phases later**, once all 5 phases existed to eval — see `tools/eval_algorithms.py`: 11 hand-picked cases spanning every phase, run manually against the real chat model (not `pytest`/CI, since it costs real API money and isn't fully deterministic). First run caught nothing broken, but did surface that the harness's own checks needed to account for LaTeX-formatted math output (`\Theta(n \log n)` vs. a naive literal-Unicode-Θ substring check) — a reminder that hand-written eval checks need their own debugging pass before they're trustworthy.
...
**Extended (2026-09-15) — Dijkstra's shortest path**: `graph_algorithm_tracer.py`'s `trace_dijkstra` reuses the same `GraphStructure`/`GraphEdge.weight` data as BFS/DFS, adding a priority-queue shortest-path computation with per-step relaxation logging. Refuses to run (returns `None`, no fabricated block) if *any* edge connecting two known nodes has `weight: null` — a partial-weight graph could silently produce a wrong shortest path rather than an honestly-declined one. Verified with 22 unit tests total for this module (including a classic case where the direct A-B edge is *not* the shortest path — A-C-B beats it) and end-to-end against the live Docker stack with a real hand-drawn weighted graph. That live test caught the missing-weight refusal actually mattering in practice: the VLM's real extraction dropped one edge's weight, no verified block was injected as a result, and the model correctly flagged the missing weight and stated its assumption explicitly rather than confidently asserting a wrong shortest-path answer — the safety behavior working exactly as designed under real, imperfect extraction.

**Not yet shipped from Phase 4**: BFS/DFS/Dijkstra only (no Bellman-Ford, Floyd-Warshall, Kruskal, Prim, or topological sort — §13's fuller catalog), no dynamic programming, greedy-algorithm, or correctness-proof support at all (§17–20 remain entirely unverified, resting solely on the chat model's own reasoning like Phase 1). This phase intentionally stopped short of the full graph-algorithm catalog and DP/greedy/proofs rather than attempting full breadth — per this doc's own principle, those should wait for real usage data on what students actually ask before committing scope to them.
...
**Shipped (2026-09-15) — the explanation-modes slice**: `explanation_mode` (docs/TheoryOfAlgorithm.md §24) is a new `ChatRequest` field, only consulted for `query_mode == "algorithm"`. "University" is the default and a no-op (it's the algorithm prompt's existing baseline tone); "simple"/"rigorous"/"exam"/"socratic" each prepend an instruction block (`_EXPLANATION_MODE_INSTRUCTIONS` in `backend/app/api/v1/chat.py`) before the base algorithm prompt. Surfaced in the extension as a dropdown next to the Algorithm mode checkbox in `AskTab.tsx`, only shown when Algorithm mode is on. Deliberately stopped at explanation modes for this phase rather than also attempting rendered visualization (§30/§31) or contextual action surfacing (§38) — those need new frontend diagram infrastructure that doesn't exist yet, a materially bigger and different kind of lift than a prompt-level mode switch.

**A real bug found and fixed during live testing — socratic mode initially didn't work at all.** The first two implementations (appending the socratic instruction after the base prompt; then also masking the verified blocks as an "ANSWER KEY — DO NOT REVEAL YET" section instead of using their normal headers) both failed in a live test: the model gave the full derivation immediately regardless, even once the verified blocks were withheld from context entirely. The actual fix needed two changes together: (1) withhold the verified recurrence/trace/graph-traversal blocks from context completely when `explanation_mode == "socratic"`, rather than relabeling them, since the model narrated a full answer from the raw data regardless of how the section was labeled; and (2) **prepend** the socratic instruction *before* the base algorithm prompt (not append it after), with explicit "OVERRIDE — this rule takes precedence over every other instruction below" framing — appending it after was reliably ignored in favor of the base prompt's "never state a complexity class without deriving it" instruction. Verified across two live conversation turns: turn 1 asked a single guiding question with zero derivation, turn 2 built the next question on the student's stated progress instead of restarting. The lesson worth keeping: a mode instruction added after a strongly-worded existing instruction is not reliably followed by asking nicely — ordering and explicit precedence-declaration mattered more than the wording itself.

**Not yet shipped from Phase 5**: no rendered/interactive recursion trees, DP tables, or graph traversal animations (§30/§31 — still ASCII/text only, from Phases 2–4), no contextual action surfacing in the UI (§38's "Recurrence detected → Solve" quick-action buttons), no automated verification that "simple"/"rigorous"/"exam" modes actually produce meaningfully different output beyond one live spot-check each — only socratic mode's correctness was verified rigorously (because it was the one that broke), and only against real usage would the others' quality actually be known.
