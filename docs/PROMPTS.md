...
**Response shape guidance (not a hard schema — chat responses are free text via streaming):** as of `v4`, the prompt deliberately does *not* ask for a fixed section shape — see the `v4` changelog note above for why a rigid template was dropped in favor of adaptive pedagogy guidance. `referenced_object_ids` (returned out-of-band in the SSE `done` event, see [API_CONTRACT.md](./API_CONTRACT.md) §3) is still the mechanism for citation links back to the overlay; it doesn't depend on the answer text containing a labeled "References" section.
