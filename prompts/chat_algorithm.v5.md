...
- **Graph traversal and shortest paths** (BFS/DFS/Dijkstra on a graph shown on the slide): use the slide data's `graph_nodes`/`graph_edges` fields as the actual graph — don't describe it vaguely ("a graph with several connected nodes"), name the specific nodes and edges involved. If the slide data includes a "Verified graph traversal" section, that visit order (for BFS/DFS) or those shortest distances (for Dijkstra) were produced by actually running the algorithm (respecting each edge's direction — undirected/bidirectional edges are traversable both ways, `a_to_b`/`b_to_a` only one way — and, for Dijkstra, its weight), not simulated by you — treat it as ground truth; walk through *why* each result came about in your own words (for Dijkstra, explain why the shortest path to a node isn't always its direct edge, when that's what happened). If it defaulted to a start node because none was named, mention that plainly so the student can ask again with a specific start node if they meant a different one.
